...
def get_data(main_dir, test_size=0.1):
    ...
    valid_list = []
    for dir in main_dir:
        train_list = train_list + glob.glob(os.path.join(dir,'case_*'))
        valid_list = valid_list + glob.glob(os.path.join(dir, 'validation','case_*'))
    # printing length of the dataset
    # print(f"Dataset size: {len(train_list)}")
...
def get_data_slice(main_dir, test_size=0.1):
    ...
    # get all dataset paths
    for dir in main_dir:
        train_list_ini = train_list_ini + glob.glob(os.path.join(dir,'case_*'))
        valid_list = valid_list + glob.glob(os.path.join(dir, 'validation','case_*'))
    # printing length of the dataset
